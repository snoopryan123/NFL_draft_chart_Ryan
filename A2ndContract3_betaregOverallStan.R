...
# plot_full_post_density_rdsall
ggsave("plots_overall/plot_post_density_full_rdsall.png", width=7, height=5)
...
plot_full_post_density_rdsall_SE = 
  ...
  geom_line(aes(y=density_times_bp_M), linewidth=1) +
  geom_ribbon(aes(ymin = density_times_bp_L, ymax=density_times_bp_U)) +
  xlab("apy cap pct") +
  ylab("density") +
  labs(title = "density of performance outcome Y") +
  scale_color_discrete(name = "draft pick") +
...
# plot_full_post_density_rdsall_SE
ggsave("plots_overall/plot_post_density_full_rdsall_SE.png", width=7, height=5)
...
get_df_V_G <- function(
    G_func, desc="", type="", surplus=FALSE, printme=TRUE, SE=FALSE, q=NA
) {
  V_G <- function(x) {
  ...
  df_V_G = df_V_G %>% mutate(desc = desc, type = type, surplus = surplus, q=q)
  df_V_G
}
...
plot_VG = 
  ...
  scale_x_continuous(breaks=seq(1,32*9,by=32*2))
# plot_VG
ggsave("plots_overall/plot_G_valueCurves.png",width=9,height=4.5)
...
# plot_post_surplus_density_full_rd1
ggsave("plots_overall/plot_post_surplus_density_full_rd1.png", width=7, height=5)
...
# plot_post_surplus_density_full_rdsall
ggsave("plots_overall/plot_post_surplus_density_full_rdsall.png", width=7, height=5)
...
### check that V_id(x) matches E(S|x)
df_post_summary_density_surplus
df_plot_surplus
...
df_plot_G_id_surplusv = 
  bind_rows(
    ...
    ,
    df_plot_surplus %>%
      select(draft_pick, surplus_M) %>%
      mutate(surplus_M1 = surplus_M/first(surplus_M)) %>%
      rename(V_G = surplus_M, V_G1 = surplus_M1) %>%
      mutate(desc = "E[S|x]")
  ) 
df_plot_G_id_surplusv
df_plot_G_id_surplusv %>%
...
### for paper and CMSAC24 slides
df_plot_V_G_S_A_2A = 
  bind_rows(
    # df_V_G_id_S %>% mutate(desc = "expected\nsurplus\nvalue"),
    df_plot_Massey_Thaler_0 %>% rename(V_G1 = surplus) %>% select(draft_pick, V_G1) %>% 
...
plot_V_G_S_A_2A = 
  df_plot_V_G_S_A_2A %>%
  mutate(q1 = percent(q, perc_digits_S)) %>%
  # filter(draft_pick < 255) %>%
  ...
  scale_x_continuous(breaks=seq(1,32*9,by=32*2))
# plot_V_G_S_A_2A
ggsave("plots_overall/plot_G_surplusValueCurves_2A.png",width=8,height=4.5)
