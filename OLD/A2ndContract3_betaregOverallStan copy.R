...
#   geom_ribbon(aes(ymin = perf_EV1_L, ymax = perf_EV1_U), fill="darkslategray2") +
...
#        plot_tail_probs_relative_2, width=8, height=4)

plot_tail_probs_relative_2_SE = 
  df_post_summary_tail_prob_1 %>%
  mutate(q1 = formattable::percent(q,perc_digits)) %>%
  ggplot(aes(x = draft_pick, y = v1)) +
  geom_ribbon(aes(ymin = tail_prob_1_U, ymax=tail_prob_1_L, fill = factor(q1)), 
...
df_V_G_step_S1 = get_df_V_G(
  G_func=G_step_func(q=q_), desc=paste0("g(y) = 1{y>",q_,"}"), surplus = TRUE, q=q_, SE=T
)
df_V_G_step_S1
q_ = q_grid[2]
# q_ = 0.05
...
plot_V_G_S_A_2A = 
  ...
  geom_ribbon(aes(ymin = V_G_L1, ymax=V_G_U1, fill=factor(q1)), data = . %>% filter(!is.na(q1)), alpha = 0.35) +
